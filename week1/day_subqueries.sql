...
SELECT name, continent FROM world WHERE continent = 'South America' UNION SELECT name, continent FROM world  WHERE continent = 'Oceania' ORDER BY name
...
WITH   pog as (SELECT population FROM world  WHERE name = 'Germany') SELECT name, CONCAT(ROUND(100*population/(SELECT population FROM pog), 0), '%') as Percentage FROM world WHERE continent = 'Europe'
...
SELECT customer, amount FROM orders o WHERE amount > (SELECT AVG(amount) FROM orders WHERE status = o.status)

-- Напиши запрос который выводит средние суммы заказов по каждому статусу, но только те статусы где средняя сумма выше 50000.
SELECT status, avg_amount FROM (SELECT status,  AVG(amount) as avg_amount FROM orders GROUP BY status) as sub  WHERE avg_amount > 50000

-- То же самое задача который сверху через HAVING 
SELECT status, AVG(amount) AS avg_amount FROM orders GROUP BY status HAVING AVG(amount) > 50000
